...
def floyd(data_road, data_cross, crossId):
    ...
    for data in data_road:
        ...
        if data.isDuplex == '1':
            # if data.laneNum == 1:
            adj[id2index[data.startId]][id2index[data.endId]] = 450 / ((data.roadLength + data.maxSpeed*2) * data.laneNum)
            adj[id2index[data.endId]][id2index[data.startId]] = 450 / ((data.roadLength + data.maxSpeed*2) * data.laneNum)
            # else: # data.laneNum > 1:
            # adj[id2index[data.startId]][id2index[data.endId]] = (data.roadLength + data.maxSpeed) / data.laneNum
            # adj[id2index[data.endId]][id2index[data.startId]] = (data.roadLength + data.maxSpeed) / data.laneNum
            # else:
            #     adj[id2index[data.startId]][id2index[data.endId]] = 2
            #     adj[id2index[data.endId]][id2index[data.startId]] = 2
        else:
            # if data.laneNum == 1:
            adj[id2index[data.startId]][id2index[data.endId]] = 800 / ((data.roadLength + data.maxSpeed*2) * data.laneNum)
            # else:
